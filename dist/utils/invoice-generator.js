...
exports.generateInvoiceText = exports.generateInvoiceHTML = void 0;
...
const generateInvoiceHTML = (invoice) => {
    const statusColor = (0, payment_validator_1.getStatusColor)(invoice.status);
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td class="text-right">${(0, payment_validator_1.formatCurrency)(item.price)}</td>
          <td class="text-right">${(0, payment_validator_1.formatCurrency)(item.subtotal)}</td>
        </tr>
        `)
        .join('')}
      </tbody>
    ...
    <div class="summary">
      ...
      <div class="total-section">
        <div class="total-line subtotal">
          <span>Subtotal</span>
          <span>${(0, payment_validator_1.formatCurrency)(invoice.subtotal)}</span>
        </div>
        ${invoice.discount
        ? `
        <div class="total-line discount">
          <span>Discount</span>
          <span>-${(0, payment_validator_1.formatCurrency)(invoice.discount)}</span>
        </div>
        `
        : ''}
        <div class="total-line tax">
          <span>Tax (10%)</span>
          <span>${(0, payment_validator_1.formatCurrency)(invoice.tax)}</span>
        </div>
        <div class="total-line total">
          <span>Total</span>
          <span>${(0, payment_validator_1.formatCurrency)(invoice.total)}</span>
        </div>
      </div>
    </div>
...
`;
    invoice.items.forEach((item) => {
        text += `${item.name}
  Quantity: ${item.quantity} x ${(0, payment_validator_1.formatCurrency)(item.price)} = ${(0, payment_validator_1.formatCurrency)(item.subtotal)}
`;
    });
    text += `
SUMMARY
────────────────────────────────────────────────────────────────
Subtotal:        ${(0, payment_validator_1.formatCurrency)(invoice.subtotal)}
`;
    if (invoice.discount) {
        text += `Discount:        -${(0, payment_validator_1.formatCurrency)(invoice.discount)}
`;
    }
    text += `Tax (10%):       ${(0, payment_validator_1.formatCurrency)(invoice.tax)}
────────────────────────────────────────────────────────────────
TOTAL:           ${(0, payment_validator_1.formatCurrency)(invoice.total)}
────────────────────────────────────────────────────────────────
